...
## LePaisa wallet and referral setup

The wallet is now INR (₹), starts at ₹0, and the client has no manual deposit controls.
A successful referral credits ₹100 to the referrer's Supabase wallet; the referred account receives ₹0.

Before testing referrals, run `supabase/referral_wallet.sql` once in the Supabase SQL Editor for the project configured in `.env.local`.
